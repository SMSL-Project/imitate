...
class MoveSheepFromBoatToGreenLand(Task):
    def __init__(self):
        super().__init__()
        self.max_steps = 1
        self.lang_template = "Move the sheep from the boat to the green land."
        self.task_completed_desc = "done moving the sheep to the green land."
        self.additional_reset()

    def reset(self, env):
        super().reset(env)
        obj_id = env.asset_ids_dict["rigid"]["minecraft/sheep.urdf"]
        fixed_id = env.asset_ids_dict["fixed"]["minecraft/green_land.urdf"]
        base_pose = env.get_object_pose(fixed_id)

        # Define the zone for the green land
